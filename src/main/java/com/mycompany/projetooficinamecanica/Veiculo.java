...
public class Veiculo {
    ...
    public static void removerVeiculoPorPlaca(String placa) throws IOException {
        Veiculo.setListaVeiculos(JsonUtil.carregarLista("data/veiculos.json", new TypeToken<List<Veiculo>>(){}.getType())); // Atualiza a lista de veículos

        boolean removido = listaVeiculos.removeIf(v -> v.getPlaca().equalsIgnoreCase(placa));

        if (removido) {
            System.out.println("Veículo removido com sucesso.");
            JsonUtil.salvar("data/veiculos.json", listaVeiculos); // Salva após remover
        } else {
